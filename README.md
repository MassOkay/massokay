### Hi there, I'm Nishiki Masuoka 👋

慶應義塾大学 総合政策学部 (SFC) 4年 / 27卒
ソフトウェアエンジニア志望です。AI (LLM) と Web開発、そして League of Legends が好きです。
いろんなものを自分の手を動かして作ってみて、内部の仕組みや構造を理解するのが好きです。
「作ったことのあるものは直すことができる」を座右の銘に、長く使うことのできるものを作りたいです。

- 🔭 **I’m currently working on:**
  - SFCシラバスRAGシステムの構築
  - 学習机のリノベ
 
- 🌱 **Tech Stack:**
  - **Languages:** Python (FastAPI), TypeScript (Next.js), Rust
  - **AI/ML:** LLM Architecture, RAG, FAISS, Pytorch, Transformer
  - **Others:** Docker, AWS

- 🎮 **Hobbies:**
  - **League of Legends:** Jungle Main (Kindred / Lee Sin) 💎
  - **Mahjong:** 🀄
  - **PC Build:** 自作PC・自作キーボード

- ⚡ **Fun fact:**
  - 自分のすべての情報をGeminiくんに読み込ませたがっています。まずはdiaryをつけるところから…
  - 2026/03/17 Google先生に握られているのが嫌なのでローカルで動かすことに執着厨…
